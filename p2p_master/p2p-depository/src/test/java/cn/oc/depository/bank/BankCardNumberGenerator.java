...
import cn.oc.depository.bank.base.GenericGenerator;
...
public class BankCardNumberGenerator extends GenericGenerator {
	private static BankCardNumberGenerator instance = new BankCardNumberGenerator();

	private BankCardNumberGenerator() {
	}

	public static BankCardNumberGenerator getInstance() {
		return instance;
	}
	...
	public BankCard generateAll() {
		BankNameEnum[] nameEnums = BankNameEnum.values();
		return generate(nameEnums[new Random().nextInt(nameEnums.length)], BankCardTypeEnum.DEBIT);
	}
	 ...
	 * @return 银行卡号
	 */
	public BankCard generate(BankNameEnum bankName, BankCardTypeEnum cardType) {
		...
		bankCard.setBankCode(bankName.getCode());
		bankCard.setBankName(bankName.getName());
		bankCard.setCardNumber(generateByPrefix(prefix));
		return bankCard;
	}
	 ...
	 */
	enum BankNameEnum {
		ICBC("ICBC", "中国工商银行", new Integer[]{622200, 622230, 622235, 622210, 622215}),
		BOC("BOC", "中国银行", new Integer[]{622752}),
		CCB("CCB", "中国建设银行", new Integer[]{622280}),
		BCOM("BCOM", "交通银行", new Integer[]{622260}),
		CITIC("CITIC", "中信银行", new Integer[]{622690}),
		CEB("CEB", "中国光大银行", new Integer[]{622655,622650,622658}),
		HXB("HXB", "华夏银行", new Integer[]{622636, 622637}),
		CMBC("CMBC", "中国民生银行", new Integer[]{622600,622601,622602,622603}),
		CGB("CGB", "广东发展银行", new Integer[]{622568}),
		PAB("PAB", "平安银行", new Integer[]{622155,622156}),
		CMB("CMB", "招商银行", new Integer[]{622578,622576,622581,439228,628262,628362,628362,628262}),
		;
